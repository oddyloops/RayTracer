...
private:
	...
	float_3 m_direction;// stripes, and waves (2D)
...
public:
	...
	__declspec(dllexport) void set_direction(float_3 direction);
	...
	__declspec(dllexport) void set_bitmap_index(int map_index,int width, int height	);
	...
	static T get_bitmap_color(int map_index,int width, int height, float u,float v,array_view<T, 3>* bitmaps) restrict(amp);
...
template<typename T>
texture_map<T>::texture_map() : m_is_null(true)
{
}

template<typename T>
texture_map<T>::texture_map(int is_bitmap, int map_type) : m_is_null(false) , m_is_bitmap(is_bitmap), m_map_type(map_type)
{
...
template<typename T>
void texture_map<T>::set_direction(float_3 direction)
{
...
template<typename T>
void texture_map<T>::set_bitmap_index(int map_index, int width, int height)
{
...
T texture_map<T>::get_value_plain(float u, float v, array_view<T, 3>* bitmaps, array_view<T, 1>* scalar_colors)  restrict(amp)
{
	if (m_is_bitmap)
	{
		return get_bitmap_color(m_map_index, m_width, m_height, u, v, bitmaps);
	}
	else {
...
template <typename T>
T texture_map<T>::get_bitmap_color(int map_index, int width, int height, float u, float v, array_view<T, 3>* bitmaps) restrict(amp)
{
	int r = height - 1 - static_cast<int>(v * height);
	int c = static_cast<int>(u * width);
	index<3> idx(map_index, r, c);
	return (*bitmaps)(idx);
}
