...
namespace rt_support
{
	namespace scene_resource_support
	{
		...
		private:
			...
			T get_value_plain(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp);

			T get_value_checkered(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp);

			T get_value_stripes(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp);

			T get_value_wavy(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp);

			static T get_bitmap_color(int map_index, int width, int height, int hor_offset, int ver_offset, float u, float v, texture_view<const T, 3> bitmaps) restrict(amp);

			static T get_scalar_color(int color_offset, int color_index, texture_view<const T, 1> scalar_colors) restrict(amp);
		...
		public:
			...
			T get_value(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors) restrict(amp); //get value based on supplied u-v coordinates
		...
		template<typename T>
		void texture_map<T>::set_stripe_width_perc(float stripe_width_perc)
		{
		...
		template<typename T>
		T texture_map<T>::get_value_plain(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp)
		{
			if (m_is_bitmap)
			{
		...
		template<typename T>
		T texture_map<T>::get_value_checkered(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp)
		{
			if (m_is_bitmap)
		...
		template<typename T>
		T texture_map<T>::get_value_stripes(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp)
		{
			if (m_is_bitmap)
			{ //it complicate things in the GPU model to have multiple bitmaps
				return get_value_plain(u, v, bitmaps, scalar_colors);
		...
		template<typename T>
		T texture_map<T>::get_value_wavy(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors)  restrict(amp)
		{
		...
		template<typename T>
		T texture_map<T>::get_bitmap_color(int map_index, int width, int height, int hor_offset, int ver_offset, float u, float v, texture_view<const T, 3> bitmaps) restrict(amp)
		{
		...
		template<typename T>
		T texture_map<T>::get_scalar_color(int color_offset, int color_index, texture_view<const T, 1> scalar_colors) restrict(amp)
		{
		...
		template<typename T>
		T texture_map<T>::get_value(float u, float v, texture_view<const T, 3> bitmaps, texture_view<const T, 1> scalar_colors) restrict(amp)
		{
			switch (m_map_type)
			{
			case map_type::plain:
