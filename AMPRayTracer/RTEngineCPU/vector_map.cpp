...
vector_map::vector_map() : all_map()
{

}

vector_map::vector_map(bool is_file_source, int map_type) : all_map(is_file_source, map_type)
{

}

void vector_map::set_bitmap_source(vector<matrix<vector<float>>> bitmap_source)
{
	m_bitmap_source = bitmap_source;
}

void vector_map::set_scalar_source(vector<vector<float>> scalar_source)
{
	m_scalar_source = scalar_source;
}

vector<float> vector_map::get_value(float u, float v) //get value based on supplied u-v coordinates
{
	switch (m_map_type)
	{
...
vector<float> vector_map::get_value_stripes(float u, float v)
{
	int stripe_index = 0;
	vector<float> uv = { u,v };
	...
	if (m_is_file_source)
	{
		matrix<vector<float>>& bmp = m_bitmap_source[stripe_index % m_bitmap_source.size()];
...
vector<float> vector_map::get_value_checkered(float u, float v)
{
	int tile_index = 0;
	...
	if (m_is_file_source)
	{
		matrix<vector<float>>& bmp = m_bitmap_source[tile_index % source_size];
		//compute horizontal and vertical indices in the bitmap
		//horizontal index starting from bottom left hence the substraction
...
vector<float> vector_map::get_value_plain(float u, float v)
{
	if (m_is_file_source)
	{
...
vector<float> vector_map::get_value_wavy(float u, float v)
{
	int stripe_index = 0;
	...
	if (m_is_file_source)
	{
		matrix<vector<float>>& bmp1 = m_bitmap_source[stripe_index % m_bitmap_source.size()];
		matrix<vector<float>>& bmp2 = m_bitmap_source[(stripe_index + 1) % m_bitmap_source.size()];
		//compute horizontal and vertical indices in the bitmap
		//horizontal index  starting from bottom left hence the substraction
		vector<float> perp_direction = { m_direction[1], -m_direction[0] };
		float v_run = vector_util::dot(uv, perp_direction);
...
vector_map& vector_map::operator=(const vector_map a)
{
